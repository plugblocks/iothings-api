...
func (db *mongo) CreateDevice(user *models.User, device *models.Device) error {
	...
	if device.SigfoxId != "" {
		count, _ := devices.Find(params.M{"metadata.sigfox_id": device.SigfoxId}).Count()
		if count > 0 {
	...
	if device.BleMac != "" {
		count, _ := devices.Find(params.M{"metadata.ble_mac": device.BleMac}).Count()
		if count > 0 {
	...
	if device.WifiMac != "" {
		count, _ := devices.Find(params.M{"metadata.wifi_mac": device.WifiMac}).Count()
		if count > 0 {
